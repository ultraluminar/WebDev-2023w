...
document.addEventListener('DOMContentLoaded',  async () => {
    ...
    class ComboBox {
        constructor(htmlElements, allOptionsTexts, dataTable) {
            ...
            this.nodes.input.addEventListener('mousedown', this.onInputMouseDown.bind(this));
        ...
        submitOption(optionText) {
            ...
            this.filterOptions();
            this.nodes.datatable.submitOption(optionText);
        }
        ...
        onInputKeyDown(event) {
            if (!this.isDropdownOpen() && event.key.match(/[A-Za-z]/)) {
                this.filterOptions();
                this.openDropdown();
                return;
            }
            if (event.key === "Enter" && this.options.filtered.length > 0) {
                this.submitOption(this.options.filtered[0].textContent);
            }
        }
